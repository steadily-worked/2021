...
`react-app` 구버전에서는 Sass를 사용하려면 추가 작업이 필요했는데, `v2` 버전부터는 별도의 추가 설정 없이 바로 사용할 수 있다.
...
새 컴포넌트를 만들어서 Sass를 사용해 보자. 우선 `node-sass`라는 라이브러리를 설치해 줘야 한다.
**`$ yarn add node-sass`**를 프로젝트 디렉토리에서 실행해 주자.
